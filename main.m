...
% for i=1:length(l)
%     X_L_0(row) = l(i,1) + randn();
%     X_L_0(row+1) = l(i,2)+ randn();
%     row = row + 2;
% end
%%
...
for i = 1:length(constrained_landmarks)
   Y_const(i) = norm(l(constrained_landmarks(i,1),:) -  l(constrained_landmarks(i,2),:))% + randn();
end
...
toc
tic
[q_constr,L_constr, del_Xs_constr] = constrained_SLAM( X_0, P_0,X_L_0, Q_estimator, R_estimator, v(k_start:k_end),...
    om(k_start:k_end), r(k_start:k_end,:),b(k_start:k_end,:),Y_const, t(k_start:k_end),d,...
       constrained_landmarks);
toc

tic
[q_penalty,L_penalty, del_Xs_penalty] = penalty_SLAM( X_0, P_0,X_L_0, Q_estimator, R_estimator, v(k_start:k_end),...
    om(k_start:k_end), r(k_start:k_end,:),b(k_start:k_end,:),Y_const, t(k_start:k_end),d,...
       constrained_landmarks);
toc
