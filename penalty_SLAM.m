function [X_op,X_L_op, del_X_list, P_states, P_landmarks]  = penalty_SLAM( X_0,P_0,X_L_0, Q, R, Y_v,Y_w, Y_r,Y_b,Y_const, t, d,constr_L)
...
C = length(Y_const); %No of constraints
T = t(2) - t(1);
...
H_obs = spalloc(N+C,K,dim*N);
H_obs_2 = zeros(N+C,M);
W_inv_motion = spalloc(K,K,dim*K);
W_obs_inv = sparse(1:N+C,1:N+C,2*N+C);
eps = 0.000001;
    
bar = waitbar(0,'Penalty SLAM iteration #1');    
while(1)
 %% System Information Matrix
    ...
    for i = 1:length(constr_L)
        ...
        H_obs_2(row,2*m_1-1:2*m_1) = GL1_constraint(const_L1,const_L2);
        H_obs_2(row,2*m_2-1:2*m_2) = GL2_constraint(const_L1,const_L2);
        W_obs_inv(row,row) = 1/0.01;
        row = row + 1;
    end
    ...
    W_inv = [W_inv_motion, sparse(K,N+C); sparse(N+C,K), W_obs_inv];
...
%         predicted_X(3) = wrapToPi(predicted_X(3));
    ...
    %Constraint
    row = N+1;
    for i =1:length(Y_const)
        m_1= constr_L(i,1);
        m_2= constr_L(i,2);
        const_L1 = X_L_op(2*m_1-1:2*m_1);
        const_L2 = X_L_op(2*m_2-1:2*m_2);
        e_op_y(row) = Y_const(i) - g_constraint(const_L1,const_L2);
        row = row + 1;
    end
    ...
    if(mag_del_X<0.01 || length(del_X_list)>20)
        waitbar(1,bar, 'Done!');
        pause(1);
        close(bar);
    ...
    waitbar(length(del_X_list)/20, bar, strcat('Penalty SLAM Iteration #',num2str(length(del_X_list)))); 
end
